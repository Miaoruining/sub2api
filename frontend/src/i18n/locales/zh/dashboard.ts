export default {
  ...
  keys: {
    ...
    copied: '已复制！',
    importToCcSwitch: 'CC Switch 导入',
    ccSwitchGuide: {
      title: 'CC Switch 一键导入',
      intro: '将当前密钥和分组配置导入 CC Switch，用于连接您的编程客户端。',
      provider: '供应商名称',
      endpoint: 'API 地址',
      model: '默认模型',
      loading: '正在获取当前分组的模型目录…',
      modelError: '暂时无法获取模型目录。请重试，或填写您已确认可用的模型 ID。',
      inactive: '当前密钥不可用。请先检查密钥状态、额度和有效期。',
      usage: '在 CC Switch 中显示余额 / 套餐余量',
      usageHint: '每 30 分钟使用此密钥向本站查询一次；导入时可在 CC Switch 中查看查询脚本。',
      secretHint: '配置包含当前 API 密钥，将交给您本机的 CC Switch。请勿分享含密钥的配置或链接。',
      stepsTitle: '导入后怎么使用',
      step1: '允许浏览器打开 CC Switch，检查配置并点击“确认导入”。',
      step2: '在对应客户端页签中，启用刚导入的供应商。导入本身不会自动切换当前供应商。',
      step3: '保存手头工作，完全退出并重新打开编程客户端，新建任务发送一句话验证。',
      launched: '已尝试打开 CC Switch。请在客户端完成确认；若没有打开，可下载安装，或使用下方手动配置。',
      launchError: '浏览器未能打开 CC Switch。请检查安装情况，或使用手动配置。',
      download: '下载 CC Switch（官方，建议 v3.20.1 或更新版本）',
      manual: '查看手动配置',
      import: '一键导入 CC Switch'
    },
    enable: '启用',
    disable: '禁用',
    nameLabel: '名称',
  ...
  modelPlaza: {
    ...
    detail: {
      ...
      longContextDisabledNote: '该分组未启用长上下文阶梯计费，超阈值请求仍按基础档计费，官方阶梯仅供参考', modelRateNote: '倍率按来源模型继承'
    },
    table: {
      model: '模型',
      input: '输入',
