<template>
 ...
 <dl v-else class="grid grid-cols-2 gap-3 rounded-xl bg-gray-50 p-4 text-sm dark:bg-dark-800">
          ...
          <div v-if="o.shared_account" class="rounded-lg border border-gray-200 p-3 text-xs leading-6 dark:border-dark-700"><p>{{ t('pool.upstream') }}</p><p v-if="o.plan_type !== 'pro'">5h: {{ percent(o.shared_account.used_5h) }} · 7d: {{ percent(o.shared_account.used_7d) }} · {{ o.shared_account.status }}</p><p class="text-gray-500">{{ t('pool.upstreamHint') }}</p></div>
