...
import { plazaProviderOrder, routeRate, routeRatesForGroup } from './catalog'
...
const groupRates = (group: ModelPlazaGroup) => routeRatesForGroup(group)
...
const options = computed(() => [...props.groups].sort((a,b) => plazaProviderOrder(a.platform)-plazaProviderOrder(b.platform)).map(group => ({ id: group.id, name: group.name, platform: group.platform, rate: effectiveRate(group), rates: groupRates(group) })))
const rates = computed(() => [...new Set(props.groups.flatMap(groupRates))].sort((a,b) => a-b))
watch(rates, values => { if (rate.value !== 'all' && !values.includes(rate.value)) rate.value = 'all' })
const filtered = computed(() => props.groups
  .filter(group => (platform.value === 'all' || group.platform === platform.value) && (groupId.value === 'all' || group.id === groupId.value) && (rate.value === 'all' || groupRates(group).includes(rate.value)))
  .map(group => ({ ...group, models: group.models.filter(model => model.name.toLowerCase().includes(search.value.trim().toLowerCase()) && (rate.value === 'all' || routeRate({ group, model }) === rate.value)) }))
  .filter(group => group.models.length > 0)
  .sort((a,b) => (groupRates(a)[0] ?? effectiveRate(a))-(groupRates(b)[0] ?? effectiveRate(b)) || a.name.localeCompare(b.name)))
</script>
