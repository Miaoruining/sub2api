      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="canDuplicate && !poolScope" @click="$emit('duplicate', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
            ...
            <button v-if="isOpenAIOAuthParent && !poolScope" @click="$emit('create-spark-shadow', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-amber-600 hover:bg-gray-100 dark:hover:bg-dark-700">
...
const props = defineProps<{ poolScope?: boolean; show: boolean; account: Account | null; anchorRect: DOMRect | null }>()
...
const hasRecoverableState = computed(() => {
  // OpenAI model/runtime blocks are in-memory and are not reflected by the
  // persisted status fields below. Keep recovery available even when the row
  // looks healthy so an operator can clear an invisible scheduler exclusion.
  return props.account?.platform === 'openai' || props.account?.status === 'error' || Boolean(isRateLimited.value) || Boolean(isOverloaded.value) || Boolean(isTempUnschedulable.value)
})
