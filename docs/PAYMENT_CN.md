...
| 参数 | 说明 | 必填 |
|------|------|------|
| **签名模式** | `普通公钥` 或 `证书`，必须与支付宝开放平台中的 App ID 配置一致 | 是 |
| **AppID** | 支付宝应用 AppID | 是 |
| **应用私钥** | RSA2 应用私钥 | 是 |
| **支付宝公钥** | 普通公钥模式填写支付宝公钥，不是应用公钥 | 普通公钥模式 |
| **应用公钥证书** | 证书模式粘贴 `appCertPublicKey_*.crt` 的完整 PEM 内容 | 证书模式 |
| **支付宝公钥证书** | 证书模式粘贴 `alipayCertPublicKey_RSA2.crt` 的完整 PEM 内容 | 证书模式 |
| **支付宝根证书** | 证书模式粘贴 `alipayRootCert.crt` 的完整 PEM 内容 | 证书模式 |

证书模式会自动计算并随请求提交 `app_cert_sn` 与 `alipay_root_cert_sn`。三份证书内容和应用私钥均按敏感字段处理，管理接口不会回显明文，审计日志也会脱敏。已有未完成订单时不可切换签名模式或替换签名材料。
...
| 参数 | 说明 | 必填 |
|------|------|------|
| **验签模式** | `微信支付公钥` 或 `平台证书自动轮换`；必须与商户平台当前凭据体系一致 | 是 |
| **AppID** | 微信支付 AppID | 是 |
| **商户号（MchID）** | 微信支付商户号 | 是 |
| **商户 API 私钥** | 商户 API 私钥（PEM 格式） | 是 |
| **APIv3 密钥** | 32 位 APIv3 密钥 | 是 |
| **微信支付公钥** | 微信支付公钥（PEM 格式） | 微信支付公钥模式 |
| **微信支付公钥 ID** | 微信支付公钥 ID | 微信支付公钥模式 |
| **商户证书序列号** | 商户证书序列号 | 是 |

平台证书模式会使用商户号、商户私钥、商户证书序列号和 APIv3 密钥，通过微信支付 API 自动下载并定期轮换平台证书；无需手工上传微信支付平台证书。商户私钥和 APIv3 密钥仍按敏感字段加密保存，管理接口不会回显明文。
