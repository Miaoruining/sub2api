...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	compositeRouteResolver := service.ProvideCompositeRouteResolver(compositeModelRouteRepository, groupRepository)
	...
	modelPlazaService := service.ProvideModelPlazaService(channelRepository, groupRepository, pricingService, billingService, modelPricingResolver, compositeModelRouteRepository)
	...
	poolDynamicRefreshService, err := service.ProvidePoolDynamicRefreshService(poolRepository, openAIQuotaService)
	if err != nil {
		return nil, err
	}
	poolOrderHandler := handler.ProvidePoolOrderHandler(poolRepository, apiKeyAuthCacheInvalidator, billingCacheService, subscriptionService, poolDynamicRefreshService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, lotteryHandler, poolOrderHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
