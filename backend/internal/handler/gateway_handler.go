...
func (h *GatewayHandler) handleStreamingAwareErrorWithCode(c *gin.Context, status int, errType, code, message string, streamStarted bool) {
	if streamStarted {
		...
		errType = service.NormalizeAnthropicErrorType(errType)
...
func (h *GatewayHandler) GeminiCountTokens(c *gin.Context) {
	...
	if decision := h.checkSecurityAudit(c, reqLog, apiKey, subject, service.ContentModerationProtocolGemini, publicModel, body); decision != nil && !decision.AllowNextStage {
