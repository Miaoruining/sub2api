...
func RegisterGatewayRoutes(
	...
	var poolRepo service.PoolRepository
	if h.PoolOrder != nil {
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), poolQuota, autoGroup, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), poolQuota, autoGroup, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	// Antigravity 模型列表
	r.GET("/antigravity/models", gin.HandlerFunc(apiKeyAuth), poolQuota, autoGroup, requireGroupAnthropic, h.Gateway.AntigravityModels)
	...
	antigravityV1Beta.Use(poolQuota)
